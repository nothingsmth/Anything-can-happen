...
SUPPORTED_RUNTIME = "anything_can_happen"
...
def _get_anything_can_happen_section(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if strategy_cfg is None:
        raise ValueError("Missing strategies.anything_can_happen section in config.json")
    if not isinstance(strategy_cfg, dict):
        raise ValueError("strategies.anything_can_happen must be an object")
    return strategy_cfg
...
def load_anything_can_happen_config() -> tuple[ExchangeConfig, AnythingCanHappenConfig]:
    return _load_anything_can_happen_config(_load_config_file())


def _load_anything_can_happen_config(
    cfg: dict[str, Any],
) -> tuple[ExchangeConfig, AnythingCanHappenConfig]:
    conn = cfg.get("connection", {})
    if not isinstance(conn, dict):
        raise ValueError("config.json field 'connection' must be an object")
    strat = _get_anything_can_happen_section(cfg)

    exchange = _build_exchange_config(conn)
    strategy = AnythingCanHappenConfig(
    ...
    _validate_anything_can_happen_config(strategy)
    return exchange, strategy


def _validate_anything_can_happen_config(cfg: AnythingCanHappenConfig) -> None:
